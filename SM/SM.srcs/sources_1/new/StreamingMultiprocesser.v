...
generate
    ...
    StreamProcessor _SP(
        .clk(CLK), .instruction(InsOutOfMulInsUnit[gv_i]), 
        .Return(ReturnFMulNet[gv_i]), .MemoryReadData(DataFMulNet[gv_i]),
        .Occupied(ValidVectorFromSP[gv_i]), .MemoryAddress(MemoryAddress[gv_i]),
        .MemoryWriteData(MemoryWriteData[gv_i]), .MemoryRead(MemoryRead[gv_i]),
